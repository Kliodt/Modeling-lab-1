    ...
    if d == distriutions.DUAL_EXP:
        for i in range(n):
            if (random() < distr_info[0]):
                result.append(numpy.random.exponential(distr_info[1], 1)[0])
            else:
                result.append(numpy.random.exponential(distr_info[2], 1)[0])
...
#Пример:
print(sample)
print(genereate_sample(get_dual_exp_math_expected(0.5, koVariatsiya(sample), matOzhidaniye(sample)), 30, distriutions.DUAL_EXP))
print(genereate_sample((matOzhidaniye(sample),), 30, distriutions.EXP))
